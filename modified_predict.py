...
class BasePredictor:
    ...
    #===================================================================
    # Evaluation hand (Custom for POKER)
    def evaluate_hand(self,cards_suit, cards_rank):
        hand = None
        colour = None
        rank = None

        if len(set(cards_suit)) == 1:  # one kind of flushes
            if max(cards_rank) - min(cards_rank) == 4:  # Royal or straight flush
                if max(cards_rank) == 14:  # ROYAL FLUSH
                    hand = "!!!! ROYAL FLUSHHHHH !!!!"
                    colour = (0, 0, 255)  # Red
                    rank = 1
                else:  # STRAIGHT FLUSH
                    hand = "STRAIGHT FLUSH !!!!"
                    colour = (255, 102, 0)  # Orange
                    rank = 2
            elif max(cards_rank) - np.sum(sorted(cards_rank)[:-1]) == 0:  # [A*, 2*, 3*, 4*, 5*]
                hand = "STRAIGHT"
        ...
        elif len(set(cards_rank)) == 2:  # Four of a kind / Full house
            _, counts_elements = np.unique(cards_rank, return_counts=True)
            if 4 in counts_elements:  # FOUR OF A KIND
                hand = "FOUR OF A KIND!!"
                colour = (255, 255, 0)  # Yellow
                rank = 3
            else:  # FULL HOUSE
                hand = "FULL HOUSE !!"
                colour = (255, 153, 51)  # Gold
                rank = 4

        elif len(set(cards_rank)) == 5 and (max(cards_rank) - min(cards_rank) == 4 or (max(cards_rank) - np.sum(sorted(cards_rank)[:-1]) == 0)):
            hand = "STRAIGHT"
            colour = (60, 255, 120)  # Teal Green
            rank = 6

        elif len(set(cards_rank)) == 3:  # Three of a kind / Two pairs
            _, counts_elements = np.unique(cards_rank, return_counts=True)
            if 3 in counts_elements:
                hand = "Three of a kind"
                colour = (204, 102, 255)  # Purple
                rank = 7
            else:
                hand = "2 Pairs"
                colour = (204, 204, 0)  # Mustard
                rank = 8
        ...
        for batch in self.dataset:
            ...
            for i in range(len(im)):
                ...
                if verbose or self.args.save or self.args.save_txt or self.args.show:
                    ...
                    #===================================================================
                    # Modify output on console
                    filtered_s = s.split(": ")[1]
                    splited_s = filtered_s.replace(",","").replace("/","").replace("x","").replace("s","").split()

                    opened_cards = [c for c in splited_s if len(c) < 4 and (c[0].isdigit() or c[0] in 'JQKA') and not c.isdigit()]
                    print(opened_cards)
                    num_opened_cards = len(opened_cards)
                    highest_rank = 11
                    out_hand = None
                    out_color = None
                    if num_opened_cards:
                        if num_opened_cards == 2:
                            cards_suit = [c[-1] for c in opened_cards]
                            cards_rank = [
                                        11 if c[:-1] == "J" else 
                                        12 if c[:-1] == "Q" else 
                                        13 if c[:-1] == "K" else 
                            ...
                            if len(set(cards_rank)) == 1:
                                out_hand = "Pair"
                                out_color = (180, 180, 180)
                            else:
                                out_hand = "High card =(("
                                out_color = (140, 140, 140)
                        elif num_opened_cards in range(5,8): # 5 cards
                            list_five_cards = combinations(opened_cards, 5)

                            for five_cards in list_five_cards:
                                cards_suit = [c[-1] for c in five_cards]
                                cards_rank = [
                                        11 if c[:-1] == "J" else 
                                        12 if c[:-1] == "Q" else 
                                        13 if c[:-1] == "K" else 
                                ...
                                rank, hand, color = self.evaluate_hand(cards_suit,cards_rank)
                    ...
                    #===================================================================
                if self.args.show:
                    self.show(p, out_hand, out_color)

                if self.args.save:
                    self.save_preds(vid_cap, i, str(self.save_dir / p.name), out_hand, out_color)
    ...
    # Modify params
    def show(self, p, hand, color):
        im0 = self.annotator.result()

        #===================================================================
        # Put hand on the screen
        _, img_width = im0.shape[:2]
    ...
    def save_preds(self, vid_cap, idx, save_path, hand, color):
        im0 = self.annotator.result()
        #===================================================================
        # Put hand on the screen
        _, img_width = im0.shape[:2]
